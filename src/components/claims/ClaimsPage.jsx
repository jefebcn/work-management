...
import { MINISTRY_BOTANICALS } from '../../data/ministryBotanicals.js'
import BotanicalsTab from './BotanicalsTab.jsx'
...
const TABS = [
  ...
  { key: 'ministeriale', label: 'Info Ministeriali' },
]
...
export default function ClaimsPage() {
  ...
  const isMinisteriale = activeCategory === 'ministeriale'
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-xs text-galenic-muted mt-0.5">
          Reg. UE 432/2012 · Min. Salute IT Allegato 1 (2022) · DM 10/08/2018 (Belfrit)
        </p>
      </div>
        ...
        <div className="flex gap-1 overflow-x-auto shrink-0">
          {TABS.map(tab => {
            ...
            const count = tab.key === 'tutti'
              ? CLAIMS.length
              : tab.key === 'ministeriale'
                ? MINISTRY_BOTANICALS.length
      ...
      {/* Ministeriali tab — full BotanicalsTab component */}
      {isMinisteriale ? (
        <BotanicalsTab />
      ) : (
        <>
