...
export default function IngredientRow({ computedRow }) {
  ...
  return (
    <tr className={`border-b border-galenic-border hover:bg-galenic-elevated hover:bg-opacity-30 transition-colors ${rowBg}`}>
      ...
      {/* Amount input (disabled if filler) */}
      <td className="px-4 py-2 w-36">
        <div className="flex items-center gap-1">
          <input
            ...
            step="any"
            min="0"
            value={computedRow.isFiller ? displayAmount.toFixed(3) : (fromMg(ingredient.amountMg, unit) || '')}
            onChange={handleAmountChange}
            disabled={computedRow.isFiller}
            className={[
              'w-full bg-galenic-elevated border text-galenic-primary font-mono text-sm px-2 py-1.5',
              'outline-none focus:border-galenic-accent tabular-nums',
              computedRow.isFiller
                ? 'border-galenic-border text-galenic-muted cursor-not-allowed opacity-70'
                : 'border-galenic-border',
            ].join(' ')}
          />
      ...
      <td className="px-4 py-3 tabular-nums text-center">
        ...
        {computedRow.exceedsMaxLimit && (
          <div className="text-xs text-galenic-danger mt-0.5">
